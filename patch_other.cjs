...
function applyPatch(path, webhookTarget, webhookRepl) {
    ...
    if (!content.includes(target1)) {
        console.error("Target 1 not found in", path);
        return false;
    }
    if (!content.includes(target2)) {
        console.error("Target 2 not found in", path);
        return false;
    }
    if (!content.includes(webhookTarget)) {
        console.error("Webhook target not found in", path);
        return false;
    }

    content = content.replace(target1, repl1);
    content = content.replace(target2, repl2);
    content = content.replace(webhookTarget, webhookRepl);
...
// Discounted
const discPath = '../Discounted-Cg-main/src/main.js';
const discWebTarget = `                if (carData.vin || carData.title) {
                    const sourceScraper = 'Recently Discounted';`;
const discWebRepl = `                if (carData.vin || carData.title) {
                    const baseScraper = 'Recently Discounted';
                    const sourceScraper = \`\${baseScraper} - Page \${pageToScrape}\`;`;
                    
applyPatch(discPath, discWebTarget, discWebRepl);

// Newest
const newPath = '../NEWEST-CARGURUS-main /src/main.js';
const newWebTarget = `                if (carData.vin || carData.title) {
                    const sourceScraper = pageToScrape >= 1 && pageToScrape <= 6
                        ? 'Newest 3-pager'
                        : 'Newest';`;
const newWebRepl = `                if (carData.vin || carData.title) {
                    const baseScraper = pageToScrape >= 1 && pageToScrape <= 6
                        ? 'Newest 3-pager'
                        : 'Newest';
                    const sourceScraper = \`\${baseScraper} - Page \${pageToScrape}\`;`;
                    
applyPatch(newPath, newWebTarget, newWebRepl);
